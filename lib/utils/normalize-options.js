 ...
 * @param {(function(string):string)} [options.destTransform=null] A function to transform the destination path with. Careful with this!
...
module.exports = function normalizeOptions (source, outputDir, options) {
  ...
  const destTransform = options && options.destTransform ? options.destTransform : (dest) => dest
  const toDestination =
        baseDir === '.'
          ? sourcePath => destTransform(path.join(normalizedOutputDir, sourcePath))
          : sourcePath => destTransform(sourcePath.replace(baseDir, normalizedOutputDir))
  ...
  return {
    ...
    toDestination,
    destTransform: options && options.destTransform ? options.destTransform : () => {},
